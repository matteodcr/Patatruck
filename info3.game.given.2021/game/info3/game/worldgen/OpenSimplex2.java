...
// License : CC-0
// Modifié pour adhérer aux conventions de nommage
...
public class OpenSimplex2 {
	...
	/**
	 * 2D Simplex noise, with Y pointing down the main diagonal. Might be better for
	 * a 2D sandbox style game, where Y is vertical. Probably slightly less optimal
	 * for heightmaps or continent maps, unless your map is centered around an
	 * equator. It's a subtle difference, but the option is here to make it an easy
	 * choice.
	 */
	...
	/**
	 * 3D OpenSimplex2 noise, with better visual isotropy in (X, Y). Recommended for
	 * 3D terrain and time-varied animations. The Z coordinate should always be the
	 * "different" coordinate in whatever your use case is. If Y is vertical in
	 * world coordinates, call noise3improveXZ(x, z, Y) or use noise3_XZBeforeY. If
	 * Z is vertical in world coordinates, call noise3improveXZ(x, y, Z). For a time
	 * varied animation, call noise3improveXY(x, y, T).
	 */
	public static float noise3improveXY(long seed, double x, double y, double z) {

		// Re-orient the cubic lattices without skewing, so Z points up the main lattice
		// diagonal,
		// and the planes formed by XY are moved far out of alignment with the cube
		// faces.
	...
	/**
	 * 3D OpenSimplex2 noise, with better visual isotropy in (X, Z). Recommended for
	 * 3D terrain and time-varied animations. The Y coordinate should always be the
	 * "different" coordinate in whatever your use case is. If Y is vertical in
	 * world coordinates, call noise3improveXZ(x, Y, z). If Z is vertical in world
	 * coordinates, call noise3improveXZ(x, Z, y) or use noise3improveXY. For a time
	 * varied animation, call noise3improveXZ(x, T, y) or use noise3improveXY.
	 */
	public static float noise3improveXZ(long seed, double x, double y, double z) {

		// Re-orient the cubic lattices without skewing, so Y points up the main lattice
		// diagonal,
		// and the planes formed by XZ are moved far out of alignment with the cube
		// faces.
		// Orthonormal rotation. Not a skew transform.
		double xz = x + z;
	...
	/**
	 * 3D OpenSimplex2 noise, fallback rotation option Use noise3improveXY or
	 * noise3improveXZ instead, wherever appropriate. They have less diagonal bias.
	 * This function's best use is as a fallback.
	 */
	...
	private static float noise3unrotatedBase(long seed, double xr, double yr, double zr) {
		...
		// Compute absolute values, using the above as a shortcut. This was faster in my
	...
	/**
	 * 4D OpenSimplex2 noise, with XYZ oriented like noise3improveXY and W for an
	 * extra degree of freedom. W repeats eventually. Recommended for time-varied
	 * animations which texture a 3D object (W=time) in a space where Z is vertical
	 */
	...
	/**
	 * 4D OpenSimplex2 noise, with XYZ oriented like noise3improveXZ and W for an
	 * extra degree of freedom. W repeats eventually. Recommended for time-varied
	 * animations which texture a 3D object (W=time) in a space where Y is vertical
	 */
	public static float noise4improveXYZimproveXZ(long seed, double x, double y, double z, double w) {
	...
	/**
	 * 4D OpenSimplex2 noise, with XYZ oriented like noise3fallback and W for an
	 * extra degree of freedom. W repeats eventually. Recommended for time-varied
	 * animations which texture a 3D object (W=time) where there isn't a clear
	 * distinction between horizontal and vertical
	 */
	public static float noise4improveXYZ(long seed, double x, double y, double z, double w) {
	...
	/**
	 * 4D OpenSimplex2 noise, with XY and ZW forming orthogonal triangular-based
	 ...
	 */
	public static float noise4improveXYimproveZW(long seed, double x, double y, double z, double w) {
	 ...
	 */
	public static float noise4fallback(long seed, double x, double y, double z, double w) {
	 ...
	 */
	private static float noise4unskewedBase(long seed, double xs, double ys, double zs, double ws) {

		// Get base points and offsets
		int xsb = fastFloor(xs), ysb = fastFloor(ys), zsb = fastFloor(zs), wsb = fastFloor(ws);
		float xsi = (float) (xs - xsb), ysi = (float) (ys - ysb), zsi = (float) (zs - zsb), wsi = (float) (ws - wsb);

		// Determine which lattice we can be confident has a contributing point its
		// corresponding cell's base simplex.
		// We only look at the spaces between the diagonal planes. This proved effective
		// in all of my tests.
		...
		for (int i = 0;; i++) {
			...
			// Next point is the closest vertex on the 4-simplex whose base vertex is the
			// aforementioned vertex.
			...
			// Because we don't always start on the same lattice copy, there's a special
			// reset case.
	...
	static {
		...
		float[] grad2 = { 0.38268343236509f, 0.923879532511287f, 0.923879532511287f, 0.38268343236509f,
				0.923879532511287f, -0.38268343236509f, 0.38268343236509f, -0.923879532511287f, -0.38268343236509f,
				-0.923879532511287f, -0.923879532511287f, -0.38268343236509f, -0.923879532511287f, 0.38268343236509f,
				-0.38268343236509f, 0.923879532511287f,
				// -------------------------------------//
				0.130526192220052f, 0.99144486137381f, 0.608761429008721f, 0.793353340291235f, 0.793353340291235f,
				0.608761429008721f, 0.99144486137381f, 0.130526192220051f, 0.99144486137381f, -0.130526192220051f,
				0.793353340291235f, -0.60876142900872f, 0.608761429008721f, -0.793353340291235f, 0.130526192220052f,
				-0.99144486137381f, -0.130526192220052f, -0.99144486137381f, -0.608761429008721f, -0.793353340291235f,
				-0.793353340291235f, -0.608761429008721f, -0.99144486137381f, -0.130526192220052f, -0.99144486137381f,
				0.130526192220051f, -0.793353340291235f, 0.608761429008721f, -0.608761429008721f, 0.793353340291235f,
		...
		float[] grad3 = { 2.22474487139f, 2.22474487139f, -1.0f, 0.0f, 2.22474487139f, 2.22474487139f, 1.0f, 0.0f,
				3.0862664687972017f, 1.1721513422464978f, 0.0f, 0.0f, 1.1721513422464978f, 3.0862664687972017f, 0.0f,
				0.0f, -2.22474487139f, 2.22474487139f, -1.0f, 0.0f, -2.22474487139f, 2.22474487139f, 1.0f, 0.0f,
				-1.1721513422464978f, 3.0862664687972017f, 0.0f, 0.0f, -3.0862664687972017f, 1.1721513422464978f, 0.0f,
				0.0f, -1.0f, -2.22474487139f, -2.22474487139f, 0.0f, 1.0f, -2.22474487139f, -2.22474487139f, 0.0f, 0.0f,
